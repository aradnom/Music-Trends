...
class MongoMe {
	...
	public function MongoMe ( $dbname, $collection = null ) {
		$this->dbname = $dbname;
	...
	public function find ( $properties, $fields = array() ) {
	...
	public function findOne ( $properties ) {
		if ( $this->collection && !empty( $properties ) ) {
			$this->results = $this->collection->findOne( $properties );
	...
	public function update ( $id = null, $searchProperties = null, $updateProperties, $replace = false, $options = null ) {
		if ( $this->collection ) {
			if ( $id && $searchProperties ) {
				$searchProperties["_id"] = $id;
				
				if ( $replace )
					return ( $this->collection->update( $searchProperties, $updateProperties, $options ) );
				else
					return ( $this->collection->update( $searchProperties, array( '$set' => $updateProperties ), $options ) );
			} elseif ( $id ) {
				if ( $replace )
					return ( $this->collection->update( array( "_id" => $id ), $updateProperties, $options ) );
				else
					return ( $this->collection->update( array( "_id" => $id ), array( '$set' => $updateProperties ), $options ) );
